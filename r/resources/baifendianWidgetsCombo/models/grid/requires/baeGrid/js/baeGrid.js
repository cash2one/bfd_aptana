...
//								alert('没有相应数据')
                ...
                this.pushLine = function(dom, stores, seq,index){
                    ...
                    $.each(setting['colModel'], function(i, item){
						...
						if(coldom.data('content').length>40){
							if(item.type&&item.type.indexOf('link')!=-1){
								coldom.find('a').html(coldom.data('content').substring(0,38)+'...')
							}else{
								coldom.html(coldom.data('content').substring(0,38)+'...')
							}
							coldom.qtip({
						                            content: coldom.data('content'),
						                            style: {
						                                name: 'blue'
						                            },
						                            position: {
