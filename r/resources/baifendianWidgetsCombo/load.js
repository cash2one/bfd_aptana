...
(function(){
    ...
    if ((_path.indexOf('http') != 0 && _path.indexOf('/') != 0) ||
    _path.indexOf('./') == 0) {
        _path = window.location.href.substring(0, window.location.href.lastIndexOf("/") +
        1) +
        _path
    }
    ...
    window.BaifendianWidgets = function(param){
        ...
        self.init = function(){
            ...
            typeof(jQuery) == 'undefined' ? commonLoads.unshift(_path +
            'commons/jquery.js') : null
            ...
            function _jqLoad(){
                ...
                _getSetting().done(function(data){
                    ...
                    if (data.global.dateRange.type == "fix") {
                        $L = $L.script(_path +
                        "models/DateRange/js/daterangeselectbox.js")
                    }
                    else 
                        if (data.global.dateRange.type == "dynamic") {
                            var link = document.createElement("link");
                            link.setAttribute("rel", "stylesheet");
                            link.setAttribute("type", "text/css");
                            link.setAttribute("href", _path +
                            "models/DateRange/css/datepicker.css");
                            document.getElementsByTagName("head")[0].appendChild(link);
                            $L = $L.script(_path +
                            "models/DateRange/js/datarange.all.min.js")
                        }
                    $L.wait(function(){
                        var dateRange
                        if (data.global.dateRange.type == "fix") {
                            dateRange = new DateRange($('#' +
                            data.global.dateRange['container']), data.global.dateRange['option'])
                        }
                        else {
                            dateRange = new DateRange($('#' +
                            data.global.dateRange['container']), data.global.dateRange['isCompare'], data.global.dateRange['align'])
                        }
