...
//                        }
                ...
                this.flush = function(){
                    ...
                    $.each(setting.grid.filter['colModel'], function(i, item){
                        ...
                        var input = $('<input/>', {
                            'type': 'checkbox',
                            'value': item['name']
                        }).attr('checked', setting.grid.filter.defaultCols.include(item['name']) ? true : false).click(function(){
                                ...
                                if ($(this).is(':checked') && filterListDom.find('input:checkbox:checked').size() === 7) {
                                    ...
                                    if (filterListDom.find('input:checkbox:checked').size() === 0) {
                                        ...
                                        if (setting.grid.filter.defaultCols.equal(getCols())) {
                                            ...
                                            if (setting.grid.filter.orderCols.equal(getCols())) {
                                                $(setting.grid.filter.filterModeDom).find('a:eq(1)').addClass('now')
                                            }
                                            else 
                                                if ((setting.grid.filter.goodsCols.equal(getCols()))) {
            ...
            function Grid(dateRange, filter, toggle){
                ...
                this.flush = function(resetCol){
                    var param = $.extend(true, {}, setting.grid.grid, {
                        ...
                        param['fixColModel'] = [{
                            ...
                            render: function(dom, stores){
                                ...
                                $.each(stores, function(i, item){
									...
									console.info( item[cols[0].name]+':'+v)
                                    other += v
                                })
                                if (other < 100) {
                                    data.push({
                ...
                this.init = function(){
                    $.baeGrid($.extend(true, {}, setting.grid.grid, {
                        colModel: filter.getCols()
                    }, {
                        data: {
                            begin: dateRange.begin,
            ...
            function Install(){
                ...
                this.install = function(){
					...
					self.chart = new Chart(self.dateRange, self.menuItem, self.menuItemCompare, setting);
