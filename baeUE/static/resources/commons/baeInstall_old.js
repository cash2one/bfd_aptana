    ...
    var methods = {
        init: function(opts){
            ...
            function MenuItem(){
                ...
                this.bindCompareMenu = function(){
                    if (setting.chart.type !== 'map') {
                        $(setting.chart.compareItem_renderTo).change($.proxy(function(event){
                            if ($(event.target).val() === $(event.target).find('option:first').val()) {
                                self.currentDbItem = {}
                            }
                            else {
                                $.each(self.items, function(i, item){
                                    if ($(setting.chart.compareItem_renderTo).val() === item.key) {
                ...
                this.flushCompareItem = function(){
                    if (setting.chart.type !== 'map') {
                        ...
                        $.each(this.items, $.proxy(function(i, item){
                ...
                this.flushLegend = function(){
					if(setting.chart.type !== 'map'){
                                          $('.tushi:gt(0)').hide()
						                    $('.tushi:even span').html(this.currentItem.title)
						                    if (!$.isEmptyObject(this.currentDbItem)) {
						                        $('.tushi:odd span').html(this.currentDbItem.title)
						                    }
						                    if (dateRange.mode === 'compare') {
						                        $('.tushi:eq(2)').show()
						                    }
						                    ...
						                    if (dateRange.mode === 'compare' && (!$.isEmptyObject(this.currentDbItem))) {
						                        $('.tushi:eq(3)').show()
						                    }
					}else{
						 $('.tushi').hide()
						$('.tushi:eq(4)').show()
						$('.tushi:eq(4)').find('span:first').html(this.currentItem.title+":")
					} 
                }
                this.init = function(){
                    this.flush()
                    this.flushCompareItem()
                    this.bindCompareMenu()
                }
                this.init()
            }
            ...
            function Chart(){
                ...
                this.param = {                    //								timeType:1
                }
                this.init = function(){
                    var p = {
                        typeName: menuItem.currentItem.key
                    }
                    if ($.isEmptyObject(menuItem.currentDbItem)) {
                        p['dbTypeName'] = menuItem.currentDbItem.key
                    }
                    var url = setting.chart.url + '?' +
                    $.param($.extend(true, {}, p, this.param, {
                ...
                this.reflush = function(){
                    ...
                    var url = setting.chart.url + '?' + $.param($.extend(temParam, p, this.param, {
                        begin: dateRange.begin,
                        end: dateRange.end
                    }, setting.chart.data))
                    if (setting.chart.type === 'line') {
            ...
            function Grid(){
            	 var self=this
            	 var tip=$('<div style="background-color: #FEFFF1;position:absolute;border: 1px solid #F8F3CD;width:400px;"></div>')
            	 tip.appendTo('body').hide()
            	 $(window).resize(function(){
            		 self.captalWidth=$(setting.grid.grid.renderTo).find('th.tb_index').width()
                 })
                 setting.grid.grid['beforeColSetVal'].push(function(dom,store,name){
            		 if(store[name]&&store[name].indexOf('</a>')===-1){
            			 if(store[name].length>40){
                			 dom.data('tip',store[name])
                			 store[name]=store[name].substring(0,38)+'...'
                		 }else{
                			 if(dom.data('tip')){
                				 dom.removeData('tip')
                			 }
                 ...
                 function flush_title(){
            		 $(setting.grid.grid.renderTo).mousemove(function(event){
            			 if($(event.target).is('td.tb_index')){
            				 if($(event.target).data('tip')){
            					 tip.css({
                					 left:event.clientX+10,
                					 top:event.clientY+10 	 
                				 }).html($(event.target).data('tip')).show()
            				 }else{
            					 tip.hide()
            				 }
				...
				this.colExpand=function(){
					if(setting.grid.filter.fixColModel.expand){
						setting.grid.filter.fixColModel.expand.call(self,setting)
					}
				}
                this.init = function(){
					this.colExpand()
                    $.baeToggleFilterGrid($.extend(true, setting.grid, {
                        dateRange: dateRange
                    }))
                    this.captalWidth=$(setting.grid.grid.renderTo).find('th.tb_index').width()
                    flush_title()
//                    if($(setting.grid.grid.))
                    if(setting.grid.grid.callbackInGridToggleFilterCreate){
                    	setting.grid.grid.callbackInGridToggleFilterCreate(grid)
                    }
                }
                this.reflush = function(resetCol){
                    $.baeToggleFilterGrid('reflush',resetCol)
                }
                this.changeSetting=function(f,resetCol){
                	$.baeToggleFilterGrid('changeSetting',f,resetCol)
                }
				this.flushFilter=function(){
					 $.baeToggleFilterGrid('flushFilter')
				}
                this.init()
            }
            var grid = new Grid()
