...
(function($){
    var methods = {
		...
		function Chart(dateRange,currentItem,currentDbItem){
                var self = this
                this.init = function(){
                    var p = {
                        typeName: currentItem.key
                    }
                    if ($.isEmptyObject(currentDbItem)) {
                        p['dbTypeName'] = currentDbItem.key
                    }
	...
	function Toggle(filter,grid){
		...
		modelBtn.toggle($.proxy(function(){
			...
			function ChartItem(){
				var self=this
				self.key=$('.data_select select:first').val()
				$('.data_select select:first').change(function(){
					self.key=$(this).val()
					self.change()
				})
			}
