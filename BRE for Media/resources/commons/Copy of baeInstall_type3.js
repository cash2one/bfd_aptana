        ...
        init: function(opts){
            ...
            function Grid(dateRange, chart_setting, grid_setting){
                this.flush = function(resetCol){
                    var param = $.extend(true, {}, grid_setting, {
                        data: {
                            begin: dateRange.begin,
                            end: dateRange.end
                        }
                    })
                    if (grid_setting.hasScale) {
                        param['colModel'].push({
                            colName: '占比',
                            name: 'scale'
                        })
                    ...
                    param['fixColModel'] = [{
                        colName: chart_setting.title,
                        'name': 'chart',
                        render: function(dom,stores){
                            dom.css('background-color', '#F9F9F9')
                            var data=[] 
	                    	$.each(stores, function(i, item){
	                    		if(chart_setting.type=='pie'&&i==stores.length-1){
	                    			return false
	                    		}
	                            data.push({
	                                name: item[param['colModel'][0].name],
	                                y: item[param['colModel'][1].name]
	                            })
	                        })
                            var chart = new GridChart(chart_setting.type, dom[0])
                            chart.setData(data)
                            /*if(!$('body').data(grid_setting['grid_id'])){
                             ...
                             grid_pie_dom.appendTo(dom).baePieChart(p)
                             $('body').data(grid_setting['grid_id'],grid_pie_dom)
                             $('body').data(grid_setting['grid_id']+'_chart',grid_pie_dom.data('chart'))
                             }else if(chart_setting.type==='bar'){
                             grid_pie_dom.appendTo(dom).baeBarChart(p)
                             $('body').data(grid_setting['grid_id'],grid_pie_dom)
                             $('body').data(grid_setting['grid_id']+'_chart',grid_pie_dom.data('chart'))
                             }
                             }else{
                             var grid_pie_dom=$('body').data(grid_setting['grid_id'])
                             grid_pie_dom.data('chart',$('body').data(grid_setting['grid_id']+'_chart'))
                             var xmlurl=chart_setting.url+"?"+encodeURI($.param($.extend(true,{},chart_setting.data,{begin:dateRange.begin,
                             end:dateRange.end})))
                             if(chart_setting.type==='pie'){
                             grid_pie_dom.appendTo(dom).baePieChart('setData',xmlurl)
                             }else if(chart_setting.type==='bar'){
                             grid_pie_dom.appendTo(dom).baeBarChart('setData',xmlurl)
                             }
            ...
            function Report(dateRange){
                ...
                this.flush = function(){
                    var url = setting.report.url + '?' + encodeURI($.param($.extend({}, setting.report.data, {
                        begin: dateRange.begin,
                        end: dateRange.end
                    })))
