            ...
            function Structs(){
                ...
                this.createStruct = function(stores, seq){
                        ...
                        if (this.doms.length > stores.length) {
                            ...
                            if (this.doms.length < stores.length) {
                                $.each(new Array(stores.length - this.doms.length), $.proxy(function(){
									var d=this.createRow(seq, setting.data.groupCol&&stores[0].group)
									d.appendTo(table)
                    ...
                    this.doms.each(function(i){
                    	 i%2?$(this).addClass('trodd').removeClass('treven'):$(this).addClass('treven').removeClass('trodd')
                    })
                    ...
                    $.ajax({
                        ...
                        success: $.proxy(function(data){
							if(setting.onDataLoad){
								setting.onDataLoad(data,setting)
							}
					...
					this.flushVal = (function(){
						...
						return function(initdata){
						       if(initdata&&initdata.init&&initdata.init===true&&parseInt(self.total)>self.rowList){
													$(setting['page']['renderTo']).find('ul').pagination(Math.ceil(parseInt(self.total)/self.rowList), {
															current_page:0,
															items_per_page:1,
															num_edge_entries:2,
