...
(function( $ ){
  var str='<div class="page-area2">'+
				...
				'<div class="pa2-main">'+
				'<ul class="choose2">'+
				'<li class="p current"><a  class="">该文章内图片</a></li>'+
				'<li class=""><a  class="">网络图片</a></li>'+
				'<li class=""><a  class="">本地上传</a></li>'+
				'</ul>'+
				'<div class="pic-box">'+
				'</div>'+
				...
				'<p class="note" style="display:none;">支持jpg、jpeg、gif、png格式，且文件大小不超过2M的图片</p>'+
				'</div>'+
				'</div>'+
				'</div>'																																															
  var methods = {
                init:function(options){
					...
					dom.find('.choose2 > li').click(function(){
						if(!$(this).hasClass('current')){
								...
								if($(this).index()===1){
									dom.find('p.note:eq(0)').show()
								}else if($(this).index()===2){
									dom.find('p.note:eq(1)').show()
								}
							}
					})
