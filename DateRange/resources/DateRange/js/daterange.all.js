 ...
 * 
 * @modify by 宁彬彬
 * @description 默认中文日期,在非single模式下最后一个日期页显示为最后一个日期 line 160,and  Math.floor(options.calendars/2) replaceAll with Math.floor(options.calendars-1)
 */
(function ($) {
	var DatePicker = function () {
		var	ids = {},
			...
			fill = function(el) {
				...
				var cal = $(el);
				var currentCal = Math.floor(options.calendars-1), date, data, dow, month, cnt = 0, week, days, indic, indic2, html, tblCal;
				...
				for (var i = 0; i < options.calendars; i++) {
					...
					while (cnt < 42) {
						...
						var last=new Date(2010,0,1)
						if(val>=now.valueOf() || val<=last.valueOf()){
							data.weeks[indic].days[indic2].classname.push('datepickerNotInRange');
						}
			...
			click = function(ev) {
				...
				if (el.is('a')) {
					...
					var changed = false;
					var fillIt = false;
					if (parentEl.is('td') && parentEl.hasClass('datepickerNotInRange')){
						return false;
					}
					if (parentEl.is('th')) {
						if (parentEl.hasClass('datepickerWeek') && options.mode == 'range' && !parentEl.next().hasClass('datepickerDisabled')) {
							var val = parseInt(parentEl.next().text(), 10);
							tmp.addMonths(tblIndex - Math.floor(options.calendars-1));
							...
							if(tmp.valueOf()>new Date().valueOf() || tmp.valueOf()<last.valueOf()){
								return false
							}
							if(tmp.valueOf()+6*86400000>now.valueOf()){
								for(var i=5;i>=0;i--){
									if(tmp.valueOf()+i*86400000<=now.valueOf()){
										tmp.addDays(i);
										break
									}
								}
						...
						switch (tblEl.get(0).className) {
							case 'datepickerViewMonths':
								...
								options.current.addMonths(Math.floor(options.calendars-1) - tblIndex);
								tblEl.get(0).className = 'datepickerViewDays';
								break;
							case 'datepickerViewYears':
			...
			setDate: function(date, shiftTo){
				return this.each(function(){
					if ($(this).data('datepickerId')) {
						...
						if (shiftTo) {
							options.current = new Date (options.mode != 'single' ? options.date[options.date.length-1] : options.date);
						}
						fill(cal.get(0));
					}
				});
...
// 				self.dom.find('.day_range > div:eq(1)').baeDatePicker('setDate', formatDate(new Date(self.start), 'Y-m-d'), formatDate(new Date(self.end), 'Y-m-d'))
		...
		$('body').click(function(event){
			if($(event.target).parents('.day_select').size()==0&&$('.day_range:visible').size()){
				$('.day_range:visible').each(function(){
					$(this).parents('.day_select').find('.day_range > p > button:eq(1)').click()
				})
			}
		})
        self.dom.find('.day_range > p > button:eq(0)').click($.proxy(function(){
            ...
            if ($.cookie) {
                $.cookie('baifendian_date_range_start', self.start, {
                    path: '/'
                })
                $.cookie('baifendian_date_range_end', self.end, {
                    path: '/'
                })
            }
            if (self.mode === 'single') {
    ...
    self.init = function(){
		self.events=[]
        if ($.cookie && $.cookie('baifendian_date_range_start') && $.cookie('baifendian_date_range_end')) {
			
            self.start = parseInt($.cookie('baifendian_date_range_start'),10);
            self.end = parseInt($.cookie('baifendian_date_range_end'),10);
        }
        else {
            var l = new Date()
			l.setHours(0,0,0,0)
