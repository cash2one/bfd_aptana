(function($, d3, document, window) {
	window.d3chord = function(setting) {
		...
		this.setData = function(data) {
			...
			rerender(temp_matrix, matrix, sum(matrix));
		}
		var arc = d3.svg.arc()
		...
		function rerender(temp_data, data, sum) {
			var chord = d3.layout.chord()
							.padding(.05)
							.sortSubgroups(d3.descending)
							.matrix(data);
		...
		function drawTicks(chord, svg, sum) {
			var ticks = svg.append("svg:g").attr("class", "ticks").attr("opacity", 0.1)
						.selectAll("g").data(chord.groups).enter()
						.append("svg:g").selectAll("g")
						.data(groupTicks(sum)).enter().append("svg:g").attr("transform", function(d) {
			...
			ticks.append("svg:text").attr("x", 8).attr("dy", '.35em').attr('font-size',font_size).attr("text-anchor", function(d) {
		...
		function groupTicks(sum){
			return function(d){
				var k = (d.endAngle - d.startAngle) / d.value;
				
				var total_tick = sum/72;
				var wei = 1;
				while(total_tick/10>wei){
					wei*=10;
				}
				var step = (Math.floor(total_tick/wei)*wei)
