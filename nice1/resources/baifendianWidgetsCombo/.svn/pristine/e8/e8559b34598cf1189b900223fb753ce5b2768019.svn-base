(function(d3, $, window, document) {
	window.D3Treemap = function(setting) {
		var w = setting.width||1280 - 80,
			h = setting.height||800 - 180,
			x = d3.scale.linear().range([0, w]),
			y = d3.scale.linear().range([0, h]),
			color = d3.scale.category20c(),
			root, node;

		var treemap = d3.layout.treemap().round(false).size([w, h]).sticky(true)
		treemap.value(function(d) {
			return d.size;
		});

		var svg = d3.select("#"+setting.container).append("div").attr("class", "chart").style("width", w + "px").style("height", h + "px").append("svg:svg").attr("width", w).attr("height", h).append("svg:g").attr("transform", "translate(.5,.5)");
		this.setData = function(data){
			...
			cell.append("svg:rect").attr("width", function(d) {
				...
				return color(d.parent?d.parent.name:"");
			});
