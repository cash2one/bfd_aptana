...
//                        })
        ...
        var _getSetting = function() {
                return $.ajax({
                    url:param._settingUrl||_baifendianWidgetsComboSettingURL,
                    data: {
        ...
        var _parseSetting = function(data) {
			...
			(function(){
			    ...
			    for (i = 0; j = paraString[i]; i++) {
			        urlparam[j.substring(0, j.indexOf("="))] = j.substring(j.indexOf("=") + 1, j.length);
			    }
				delete urlparam['']
			})();
			 ...
			 //重写数据Url
			 if(setting._dataUrl){
			 	_baifendianDataURL=setting._dataUrl
			 }
            var $L = $LAB.sandbox()
            ...
            $L.wait(function() {
                ...
                $.each(setting.configs, function(i, item) {
                    ...
                    loadScript(requires, $l, function() {
                        var widget = new BfdWidget[item.type]($.extend(item,{'theme':theme}), setting.global)
