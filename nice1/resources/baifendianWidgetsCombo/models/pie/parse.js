...
BfdWidget.pie = function(setting, global) {
	...
	var current_option;
	var colors = setting.rule.colors||['#67b7ff','#ffc467']
	...
	var creatChart = function(option){
		...
		option.tooltip = {
			formatter:function(){
				return '<b>'+ this.point.name +'</b>: '+ (this.point.allzero?'0 %':this.percentage.toFixed(2) +' %');
			}
		}
		$.each(option.series,function(n,i){
			$.each(i.data,function(m,j){
				j.color = colors[m]?colors[m]:undefined
			})
		})
