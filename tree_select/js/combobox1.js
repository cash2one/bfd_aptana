	...
	//									$(rander).find('li.selected').removeClass('selected')
										...
										if(!$.isEmptyObject(v)){
												if(!cateTree[path+delimiter+k]){
													...
													cateTree[path+delimiter+k]=cate
													if(c.is(':checked')){
														 cate.dom.find('li input:checkbox').attr('checked',true)
													}
...
//											     	}
	...
	//									alert(path+delimiter+k)
									   if(cateTree[path+delimiter+k].dom.find('input:radio:checked').size()==0){
												cateTree[path+delimiter+k].dom.find('input:checkbox').click()				 	
										}
									}else{
										if(cateTree[path+delimiter+k]){
	...
	//									$(this).parent().parent().parent().parent().nextAll(':visible').find('li.selected input:checkbox').attr('checked',true)
					...
					var keyStore=[]
//					$(rander).find('li:eq(0) span').click()
                   
					while($(rander).find('ul').find('li:first').not('.selected').size()!=0){
						$(rander).find('ul').find('li:first').not('.selected').find('span').click()
						$(rander).find('.area').show()
					}
					
					console.info($(rander).find('ul').find('li:first').not('.selected').size())
					while(keyStore.length!=getObjectSize(cateTree)){
						console.info(keyStore.length+':'+getObjectSize(cateTree))
						for(var k in cateTree){
//							cateTree[k].dom.html('222222')
							if($.inArray(k,keyStore)==-1){
								keyStore.push(k)
//								cateTree[k].dom.html('222222')
//								console.info(cateTree[k])
//								console.info(cateTree[k].dom.find('div.choose-box > ul').size())
//								cateTree[k].dom.find('li:first span').click()
								break;
							}
						}
					}
