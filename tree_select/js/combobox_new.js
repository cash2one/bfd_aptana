...
function cateCheckbox(json,rander,defaultData){
								...
								c.change(function(){
									if($(this).is(':checked')){
										...
										if(s.parent().hasClass('selected')){
										    if(cateTree[path+delimiter+k]&&cateTree[path+delimiter+k].dom){
										...
										if($(this).parent().parent().find('input:checkbox:checked').size()==0&&$(this).parents('.area').is(':visible')){
					...
					if($(rander).find('.area:visible ul').find('li:first input:checkbox:checked').size()!=0){
						if($(rander).find('.area:visible ul').find('li:first input:checkbox:checked').last()
						.parent().parent().parent().parent()
						.nextAll(':visible').find('input:checkbox:checked').size()==0){
