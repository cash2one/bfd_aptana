										...
										if(item.indexOf(path+delimiter+k)==0){
											...
											if(item.length>(path+delimiter+k).length){
												self.alterbuild.push(function(){
													var cate=new Categories(v,path+delimiter+k,dominit,dominit)
	...
	//									$(rander).find('li.selected').removeClass('selected')
										...
										if(!$.isEmptyObject(v)){
												if(!cateTree[path+delimiter+k]){
													var cate=new Categories(v,path+delimiter+k,false,dominit)
													cateTree[path+delimiter+k]=cate
												}else{
...
//									s.click()
					...
					if($(rander).find('ul').find('li:first input:checkbox:checked').size()!=0){
						$(rander).find('ul').find('li:first input:checkbox:checked:last')
