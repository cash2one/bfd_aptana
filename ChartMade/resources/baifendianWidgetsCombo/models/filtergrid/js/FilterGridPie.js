...
function FilterGridPie(setting, global){
    ...
    function Grid(dateRange, filter, toggle){
        ...
        this.flush = function(resetCol){
			...
			if (!$.isEmptyObject(setting.option[level].data)){
				for (var k in setting.option[level].data) {
	                    param.data.keys.push(k)
	                    param.data.values.push(setting.option[level].data[k])
	                }
			}
			...
			if(setting.option.length>1&&level!=0){
				$.each(setting.option,function(i,item){
					if(i<level){
//						param.data[setting.option[i].grid.post_name?setting.option[i].grid.post_name:setting.option[i].filter.fixColModel.name]=setting.option[i].grid.item
